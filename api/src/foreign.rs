	...
	K: Keychain + 'a,
{
	...
	/// let node_client = HTTPNodeClient::new(&wallet_config.check_node_api_http_addr, None, None).unwrap();
	///
...
macro_rules! doctest_helper_setup_doc_env_foreign {
	($wallet:ident, $wallet_config:ident) => {
		...
		let node_client =
			HTTPNodeClient::new(&wallet_config.check_node_api_http_addr, None, None).unwrap();
		let mut wallet =
