...
fn late_lock_test_impl(test_dir: &'static str) -> Result<(), libwallet::Error> {
	...
	wallet::controller::owner_single_use(Some(wallet1.clone()), mask1, None, |sender_api, m| {
		...
		// Note we're not doing this here...
		//sender_api.tx_lock_outputs(m, &slate)?;

		slate = sender_api.finalize_tx(m, &slate)?;
		println!("S3 SLATE: {}", slate);

		// But here instead, just before posting
		sender_api.tx_lock_outputs(m, &slate)?;
		Ok(())
	})?;
	...
	wallet::controller::owner_single_use(Some(wallet1.clone()), mask1, None, |api, m| {
		let (wallet1_refreshed, _wallet1_info) = api.retrieve_summary_info(m, true, 1)?;
		assert!(wallet1_refreshed);
		Ok(())
	})?;

	wallet::controller::owner_single_use(Some(wallet2.clone()), mask2, None, |api, m| {
		let (wallet2_refreshed, _wallet2_info) = api.retrieve_summary_info(m, true, 1)?;
