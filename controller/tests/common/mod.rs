...
macro_rules! create_wallet_and_add {
	($client:ident, $wallet: ident, $test_dir: expr, $name: expr, $seed_phrase: expr, $proxy: expr) => {
		...
		let $wallet = common::create_local_wallet($test_dir, $name, $seed_phrase.clone(), $client.clone());
...
pub fn create_wallet_proxy(test_dir: &str) ->
	WalletProxy<
...
pub fn create_local_wallet(test_dir: &str, name: &str, mnemonic: Option<ZeroingString>, client: LocalWalletClient) -> 
	Arc<Mutex<Box<WalletInst<
		'static,
...
{
	let mut wallet =
		Box::new(DefaultWalletImpl::<LocalWalletClient>::new(client).unwrap())
			as Box<
				WalletInst<
					DefaultLCProvider<'static, LocalWalletClient, ExtKeychain>,
					LocalWalletClient,
...
pub fn open_local_wallet(test_dir: &str, name: &str, client: LocalWalletClient) -> 
	Arc<Mutex<Box<WalletInst<
		'static,
			DefaultLCProvider<'static, LocalWalletClient, ExtKeychain>,
			LocalWalletClient,
