...
//! Early payment proof creation and verification.
...
pub(crate) fn check_proof_type(proof_type: &PaymentProofType) -> Result<(), Error> {
	match proof_type {
		PaymentProofType::Invoice | PaymentProofType::SenderNonce => Ok(()),
		_ => Err(Error::GenericError(
			"Unsupported early payment proof type".to_string(),
		)),
	}
}
...
impl EarlyPaymentProof {
	pub(crate) fn new(
		...
		// Check before indexing participant_data.
	...
	pub fn from_slate(
		...
		let (proof_type, timestamp, memo, promise_signature) = match slate.payment_proof.as_ref() {
			Some(p) => (
				p.proof_type,
				p.timestamp
		...
		proof.promise_signature = promise_signature;
		Ok(proof)
	}
...
mod tests {
	...
	fn rejects_unsupported_proofs() {
		assert!(check_proof_type(&PaymentProofType::Invoice).is_ok());
		assert!(check_proof_type(&PaymentProofType::Legacy).is_err());
		assert!(check_proof_type(&PaymentProofType::SenderNonce).is_ok());
	}
	...
	fn slate_proof_data() -> Result<(), Error> {
		...
		let receiver_key = SecretKey::from_slice(&secp, &[7; 32])?;
		assert!(slate
			.add_payment_proof_data(
				1,
				PaymentProofType::SenderNonce,
				sender_address,
				&receiver_key,
				None,
			)
			.is_err());
		let mut invalid_sender_nonce = slate.clone();
		invalid_sender_nonce.state = crate::slate::SlateState::Invoice1;
		assert!(invalid_sender_nonce
			.add_payment_proof_data(
				0,
				PaymentProofType::SenderNonce,
				sender_address,
			...
			.is_err());
		invalid_sender_nonce.participant_data.truncate(1);
		invalid_sender_nonce.num_participants = 1;
		assert!(invalid_sender_nonce
			.add_payment_proof_data(
				0,
				PaymentProofType::SenderNonce,
			...
			.is_err());
		legacy_proof.receiver_address = DalekSecretKey::from_bytes(&receiver_key.0).verifying_key();
		legacy_proof.proof_type = PaymentProofType::Legacy;
		legacy_proof.timestamp = None;
		slate.payment_proof = Some(legacy_proof.clone());
		assert!(slate
			.add_payment_proof_data(
				1,
				PaymentProofType::Invoice,
				sender_address,
				&receiver_key,
				None,
			)
		...
		legacy_proof.promise_signature = None;
		slate.payment_proof = Some(legacy_proof.clone());
		let other_sender_key = SecretKey::from_slice(&secp, &[8; 32])?;
		let other_sender_address = DalekSecretKey::from_bytes(&other_sender_key.0).verifying_key();
		assert!(slate
			.add_payment_proof_data(
				1,
				PaymentProofType::Invoice,
				other_sender_address,
				&receiver_key,
				None,
			)
			.is_err());
		assert_eq!(slate.payment_proof, Some(legacy_proof.clone()));

		let other_receiver_key = SecretKey::from_slice(&secp, &[9; 32])?;
		assert!(slate
			.add_payment_proof_data(
				1,
				PaymentProofType::Invoice,
			...
			.is_err());
		assert_eq!(slate.payment_proof, Some(legacy_proof));
		...
		slate.add_payment_proof_data(
			...
			sender_address,
			&receiver_key,
			Some(PaymentMemo::new("invoice 42".to_string())?),
		)?;
		slate.verify_payment_proof_sig(1, None)?;
		assert!(slate
			.add_payment_proof_data(
				1,
				PaymentProofType::Invoice,
		...
		slate.payment_proof.as_mut().unwrap().memo =
			Some(PaymentMemo::new("invoice 43".to_string())?);
		assert!(slate.verify_payment_proof_sig(1, None).is_err());
		Ok(())
	}
