	...
	K: Keychain,
{
	data_dir: String,
	node_client: C,
	backend: Option<Box<dyn WalletBackend<C, K>>>,
}
...
impl<'a, C, K> WalletLCProvider<C, K> for DefaultLCProvider<C, K>
where
	C: NodeClient,
	K: Keychain,
{
	...
	fn create_wallet(
		...
		let mut wallet = LMDBBackend::new(wallet_config.clone(), "", self.node_client.clone())
			.unwrap_or_else(|e| {
